...
struct ReadCoreCycleEvent final {
  ...
  std::uint64_t begin_monotonic_tick;
  std::uint64_t end_monotonic_tick;
  std::uint64_t delta_monotonic_ticks;
  std::uint64_t monotonic_frequency_hz;
  std::int64_t step_id;
  std::uint32_t context_id;
  std::uint16_t site_id;
  std::uint16_t stage_id;
};
static_assert(sizeof(ReadCoreCycleEvent) == 72);

...
extern thread_local std::int64_t read_core_cycle_step_id;
...
VLLM_RCC_PUBLIC void CommitReadCoreCyclePairedSample(
    std::uint16_t site_id,
    std::uint16_t stage_id,
...
VLLM_RCC_PUBLIC int rcc_start_selected(
    ...
    std::uint32_t context_id,
    std::uint64_t capacity) noexcept;
VLLM_RCC_PUBLIC int rcc_prepare_selected(
    std::uint16_t exact_site,
    std::uint16_t exact_stage,
    std::uint32_t context_id,
    std::uint64_t capacity) noexcept;
VLLM_RCC_PUBLIC int rcc_arm_prepared() noexcept;
VLLM_RCC_PUBLIC int rcc_set_step(std::int64_t step_id) noexcept;

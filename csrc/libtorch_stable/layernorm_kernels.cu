...
void rms_norm(torch::stable::Tensor& out,    // [..., hidden_size]
  ...
  const bool record_rms_total =
      vllm::instrumentation::ReadCoreCycleStageSelected(
          vllm::kRccGraphSite, vllm::kRccRmsNormTotal);
  const bool record_rms_prepare =
      vllm::instrumentation::ReadCoreCycleStageSelected(
          vllm::kRccGraphSite, vllm::kRccRmsNormPrepare);
  const bool record_rms_submit =
      vllm::instrumentation::ReadCoreCycleStageSelected(
          vllm::kRccGraphSite, vllm::kRccRmsNormSubmit);
  ...
  vllm::instrumentation::PublishReadCoreCycleTailBegin(
      vllm::kRccGraphSite, vllm::kRccRmsNormTail);
...
void fused_add_rms_norm(torch::stable::Tensor& input,     // [..., hidden_size]
  ...
  const bool record_fused_rms_total =
      vllm::instrumentation::ReadCoreCycleStageSelected(
          vllm::kRccGraphSite, vllm::kRccFusedRmsNormTotal);
  const bool record_fused_rms_prepare =
      vllm::instrumentation::ReadCoreCycleStageSelected(
          vllm::kRccGraphSite, vllm::kRccFusedRmsNormPrepare);
  const bool record_fused_rms_submit =
      vllm::instrumentation::ReadCoreCycleStageSelected(
          vllm::kRccGraphSite, vllm::kRccFusedRmsNormSubmit);
  const std::uint64_t total_begin =
...
#if VLLM_RCC_PROFILE_ENABLED(VLLM_RCC_PROFILE_ALL_SITES)
  vllm::instrumentation::PublishReadCoreCycleTailBegin(
      vllm::kRccGraphSite, vllm::kRccFusedRmsNormTail);

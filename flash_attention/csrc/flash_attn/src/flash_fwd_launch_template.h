...
constexpr std::uint16_t kRccFaLauncherPrepare = 42;
constexpr std::uint16_t kRccFaPrimarySubmitApi = 43;
constexpr std::uint16_t kRccFaCombineSubmitApi = 44;
...
void run_flash_fwd(Flash_fwd_params &params, cudaStream_t stream) {
    ...
    const bool record_fa_prepare =
        vllm::instrumentation::ReadCoreCycleStageSelected(
            kRccGraphSite, kRccFaLauncherPrepare);
    const bool record_fa_submit =
        vllm::instrumentation::ReadCoreCycleStageSelected(
            kRccGraphSite, kRccFaPrimarySubmitApi);
...
void run_flash_splitkv_fwd(Flash_fwd_params &params, cudaStream_t stream) {
    ...
    const bool record_fa_combine =
        vllm::instrumentation::ReadCoreCycleStageSelected(
            kRccGraphSite, kRccFaCombineSubmitApi);
    const std::uint64_t prepare_begin =
        record_fa_prepare ? vllm::instrumentation::ReadCoreCycle() : 0;
#endif
    ...
    if (params.num_splits > 1) {
        ...
        EVENK_SWITCH(is_even_K, IsEvenKConst, [&] {
            if (params.num_splits <= 2) {
                RCC_FA_COMBINE_LAUNCH(1);
            } else if (params.num_splits <= 4) {
                RCC_FA_COMBINE_LAUNCH(2);
            } else if (params.num_splits <= 8) {
                RCC_FA_COMBINE_LAUNCH(3);
            } else if (params.num_splits <= 16) {
                RCC_FA_COMBINE_LAUNCH(4);
            } else if (params.num_splits <= 32) {
                RCC_FA_COMBINE_LAUNCH(5);
            } else if (params.num_splits <= 64) {
                RCC_FA_COMBINE_LAUNCH(6);
            } else if (params.num_splits <= 128) {
                RCC_FA_COMBINE_LAUNCH(7);
            }
            C10_CUDA_KERNEL_LAUNCH_CHECK();
        });
    }
